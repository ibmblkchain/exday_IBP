...
**본 실습에서는 많은 소프트웨어들이 제공된 리눅스 서버에 이미 설치가 되어 있습니다.** 그 이유는 설치 시간을 최소하하고, 설치 등에서 발생할 수 있는 네트워크 속도 저하 문제 또는 기타 리스크 등을 제거하기 위해서입니다. 그러므로 내용을 잘 읽으면서 불필요한 작업을 중복 수행하지 않도록 조심해 주십시오.

**리눅스 서버 접속 정보를 IBM 담당자에게 문의해 주십시오**
...
**[Step 03]** 다음 명령어를 수행하여 GoLang 을 설치하십시오.
...
**[Step 04]** 아래 명령어를 통해 Go 가 정상적으로 설치되었는지 확인하세요. 물론 `go version` 의 출력값은 운영체제에 따라 다를 수 있습니다.
...
**[Step 05]** 실습을 진행할 디렉토리를 생성한 후 hello.go 파일을 생성하십시오.
...
**[Step 06]** 이제 아래 샘플을 hello.go 파일에 기입한 후 저장합니다. (putty 콘솔창에서 붙여넣기는 마우스 우클릭)
