...
**[Step 02]** 위 예시 및 아래의 상세 설명을 참조하여 `marbles_tls.json` 파일을 입맛에 맞게 수정하십시오.

config 파일은 위 예시에서 언급된 모든 필드(fields) 를 반드시 가지고 있어야 합니다. **잠깐만여!!** `cred_filename` 필드는 올바른 creds 파일명으로 세팅되어 있어야 합니다. (해당 creds 파일이 존재하는지 확인하시고 올바른 설정값이 세팅되어 있는지 훑어보십시오.)
