...
SECTION 4.	UNIT TESTING (OPTIONAL)	27
4.1.	TESTING OVERVIEW	27
4.2.	TESTING READ	28
4.3.	TESTING ADD MARBLE	30
4.4.	TESTING CHANGEOWNER	31
4.5.	TESTING DELETE	32
NOTICES	34
...
1.1 시작하기
다음 경로로 이동하십시오.
~/workspace/fabric-getting-started/release/samples/workshop/fabric-samples/first-network 
...
Section 1.	Introduction to Chaincode
...
GoLang 에 대한 자세한 내용은 여기를 참고하십시오:  https://gobyexample.com/.
...
e.	Testing overwrite
끝없는 반복이네요. 우리는 계속 새 함수를 작성하고, 테스트하는 과정을 반복할 것입니다.
...
a.	구조체 만들기(Marshalling)
지금 우리는 어느 정도 개발된 구슬(Marbles) 체인코드를 갖고 있습니다. 이제 데이터에 해당되는 부분을 좀더 복잡한 object 로 바꾸어 봅시다. GoLang 에서는 structs 라는 자료형을 통해 구조화된 데이터를 정의합니다. 우리는 이 structs 가 JSON 메타데이터를 표시하는 태그를 가진다는 것을 알게 될 것입니다. 그리고 나중에 이 구조체(structs) 를 byte 포멧으로 포장(marshal) 할 것이며, 우리는 이것들의 태그를 읽어서 각각의 데이터 key 가 실제로 어떤 data 로 매핑되어 있는지 알 수 있을 것입니다.
