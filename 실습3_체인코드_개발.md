...
append 트랜잭션을 overwrite 소스 아래에 작성하십시오.
...
이제 추가한 함수들이 올바르게 라우팅 될 수 있도록 Invoke 의 if 문을 아래와 같이 구성합시다.
...
완료하였다면 변경된 소스를 블록체인에 반영하기 위해 파일을 저장하십시오.


### e. Testing overwrite

끝없는 반복이네요. ~~내 인생처럼..~~ 우리는 계속 새 함수를 작성하고, 테스트하는 과정을 반복할 것입니다.
cli 도커 컨테이너에 접속해 있는 콘솔 창에서 아래 명령어를 수행하여 변경된 체인코드를 반영(설치 / 업그레이드) 하십시오.
...
read 쿼리문을 수행하여 데이터가 정상적으로 수정(overwrite - 엄밀히 말하면 덮어쓰기) 되었는지 확인하십시오.
...
따라서 overwrite 를 이용해서 새로운 key 를 추가한 다음에 append 함수를 써서 데이터를 수정해 보십시오. 
...
데이터를 삭제하는 로직을 구현하기 위해 아래 delete 함수를 소스의 마지막에 추가하십시오.
...
그리고 delete 함수를 Invoke 의 if 구문에 추가하십시오. 
...
**I called overwrite! Then I called append!** 이 결과로 출력되어야 지금까지의 실습을 정상적으로 수행한 것입니다.
...
Init 값은 코드가 업그레이드 될때마다 리셋됩니다.~~우리가 그렇게 짰음~~ Init 이 호출되면서 key 값이 "Test" 인 데이터는 늘 Hello, World! 로 초기화되고 있는 상태임을 기억하십시오. 반면에 key 값이 "AnotherKey" 인 데이터는 코드의 업그레이드에 영향을 받지 않고 있다는 점을 비교해 보면 이해가 더 잘 되겠죠?

이제 delete 함수를 호출하여 데이터를 삭제합니다.
...
[step xx] ‘main’ 함수 위에 새로운 자산의 형태를 structs 로 작성하십시오.
...
[step xx] 그리고 stub.PutState("Test", []byte("Hello, World!")) 라인을 Init 에서 제거하세요.
...
Init 함수는 코드가 수정(upgrade) 될 때마다 호출되었습니다. 즉 Init 함수에서는 영구적으로 보존되고 유지되어야 할 값에 대해서는 값의 세팅을 허용해서는 안됩니다. 코드가 수정될때마다 초기화되버리기 때문입니다. 따라서 권장하는 best practice 는 필요에 따라 수동으로 호출할 수 있는 함수를 별도로 생성하고, 그 함수안에 dummy 데이터를 로딩할 수 있는 코드를 삽입하는 것입니다. 

[step xx] 그것을 위해 Invoke 함수 내의 if 문에 initMarbles 를 라우팅하는 코드를 추가하십시오.
...
그리고 initMarbles 함수 작성 시 라이브러리 json 및 strconv 를 새롭게 사용하였습니다. 따라서 import statement 에 주석 처리되어 있던 부문을 제거해 주십시오. 아래와 같아야 합니다.
...
[step xx] 소스 코드를 저장한 후 cli 도커 컨테이너 콘솔 창에서 코드를 업그레이드하고 initMarbles 를 호출해 봅시다.
...
append 함수를 아래와 같이 **changeOwner 함수로 교체 (기존 append 함수를 삭제하고 changeowner 를 새로 작성)**하십시오.
...
Append 에서 changeOwner 로 라우팅을 변경하기 위해 Invoke 를 아래와 같이 수정하십시오.
...
마지막으로 append 함수에서 사용하였던 bytes 라이브러리는 더이상 사용하지 않기 때문에 import statement 에서 제거합니다.
...
read 로 조회하여 소유자가 정상적으로 변경되었는지 확인합니다.
...
overwrite 함수를 다음과 같이 **addMarble 로 교체**하십시오.
...
Overwrite 를 addMarble 로 교체하기 위해 **Invoke 의 if 문을 수정**하십시오.
```go
if function == "read" {
	return t.read(stub, args)
...
드디어 코드 작성을 완료했습니다... 마지막으로 테스트를 하고 마치겠습니다! 
